...
from loop.socs import AREA_BUDGET_KB, AREA_BUDGET_MM2, POWER_BUDGET_W, REGRESSION_TOLERANCE
...
def power_split(knobs, metrics, workloads):
    ...
    access pays its level's read energy, every fill its write energy, every instance leaks
    ...
    for workload in workloads:
        ...
        for level, sets_knob, ways_knob in LEVEL_SHAPES:
            ...
            joules[level] += leak_mw * 1e-3 * chip.instances(level) * seconds
...
def violations(metrics, workloads, stock_metrics):
    ...
    if POWER_BUDGET_W is not None and metrics["watts"] > POWER_BUDGET_W:
        broken.append("power {:.2f} W over the {} W cap".format(metrics["watts"], POWER_BUDGET_W))
    if REGRESSION_TOLERANCE is not None:
        for index, workload in enumerate(workloads):
            # On a chip with several cores every core runs its own program, and the mix's
            # mean hides a program starved for the others' benefit. The floor therefore
            # goes under each core, not under the mix.
            checks = []
            if socs.CORES > 1:
                for core in range(socs.CORES):
                    where = "W{} core {}".format(index + 1, core)
                    key = "{}:core{}:ipc".format(workload, core)
                    checks.append((where, key))
            else:
                checks.append(("W{}".format(index + 1), "{}:ipc".format(workload)))
            for where, key in checks:
                ...
                if got < floor:
                    broken.append("{} slower than the stock ({:.3f} against {:.3f})".format(where, got, floor))
    return broken
...
def budget_text(knobs):
    ...
    or a report line: "3.07 of 4.0 mm2 of silicon" or "9000 of 9216 KB in L2 + LLC"."""
    if AREA_BUDGET_MM2 is not None:
