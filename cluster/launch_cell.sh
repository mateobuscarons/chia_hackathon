...
# Env passed through: SEEDS, FIRST_SEED, BUDGET, PARALLEL_RUNS, ANALYST_MODEL, MEMORY_PATH.
...
if [ "${SMOKE_FIRST:-0}" = "1" ]; then
  SEEDS=1 .venv/bin/python -m loop.search smoke ${TAG} > results/smoke_${TAG}.log 2>&1
...
.venv/bin/python -m loop.search $CELL $TAG $ARMS > $LOG 2>&1
.venv/bin/python -m loop.search score results/runs/${CELL}_${TAG}.json >> $LOG 2>&1
