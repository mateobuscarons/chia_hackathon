...
chip class. Three kinds of fact, each derived once:
  ...
  the card        core width, ROB, clock, the bandwidth the memory channels deliver
                  per cycle, what a DRAM service costs in those cycles
...
CACTI_CACHE = os.environ.get("CACTI_CACHE", "results/cacti_{}nm.json")   # another machine's CACTI file scores that machine's runs

# ChampSim's topology, not a chip's: one core, three levels, each built once.
LEVELS = ["L1D", "L2C", "LLC"]
...
def revision():
    ...
    if "revision" not in _base_cache:
        text = json.dumps([socs.CHIP, socs.PROFILE, socs.FREQUENCY_MHZ, socs.PROCESS_NM,
                           socs.AREA_BUDGET_KB], sort_keys=True)
        digest = 0
...
def chip_text_diff():
    ...
    for section in sorted(socs.PROFILE):
        fields = ["{} {}".format(field, socs.PROFILE[section][field])
                  ...
                  if not (section in LEVELS and field in knob_fields)]
        if fields:
            parts.append("{}: {}".format(section, ", ".join(fields)))
    return "Chip {} (one core; area budget {} KB for L2 + LLC): {}".format(
        socs.CHIP, socs.AREA_BUDGET_KB, "; ".join(parts))
...
def chip_text():
    ...
    facts = card()
    return "\n".join([
        "Core: {}-wide issue ({}-wide fetch), ROB {}, LQ {} / SQ {}, {:.1f} GHz, {} nm.".format(
            facts["issue_width"], facts["fetch_width"], facts["rob"], facts["lq"], facts["sq"],
            facts["frequency_ghz"], facts["process_nm"]),
        "Memory: {} channel{} x {} MT/s x {} B = {:.1f} B per cycle; a DRAM service costs "
        ...
        "Levels: {}, one of each.".format(", ".join(LEVELS)),
        budget_text(facts),
    ])
...
def budget_text(facts):
    ...
    if facts["area_budget_mm2"] is None:
        return "Area: {} KB of L2 + LLC data.".format(facts["area_budget_kb"])
...
def leakage_w(level, sets, ways):
    """What one level leaks whether or not it hits, in watts."""
    return energy(level, sets, ways)[2] / 1000.0
...
def area_mm2(level, sets, ways):
    """The silicon one level costs."""
    return _characterised(level, sets, ways)[1]
