...
result table when they can and from ChampSim (council_loop.simulate) when they cannot. The
...
from council_loop import chip                         # the chip this run tunes, as numbers
from council_loop.simulate import (ALL_CACHES, KNOB_LOCATION, METRICS_VERSION, SimulatorInterrupted, build_binary,
                           build_config, run_simulation)
from council_loop.space import AREA_BUDGET_KB, POWER_BUDGET_W, SEARCH_SPACE, budget_text, config_name, in_space, silicon_mm2, typed_knobs, power_split, watts, within_budget
...
def workload_of(entry):
    """What one workload of a cell is: one trace, named by its file."""
    return short_name(entry), [entry]


def workloads_for(programs):
    """What a design is measured on: one workload per program in the cell."""
...
class ChampSimProblem:
    ...
    def simulate_here(self, knobs):
        ...
        try:
            ...
            binary_path = build_binary(config, CHAMPSIM_ROOT)
...
def stock_design():
    ...
    # The search starts here, so a value the space does not hold makes every design built
    # from the chip illegal - which reads as an empty design space rather than as a typo in
    # the chip's entry. Say which knob it was instead.
    outside = []
    for knob in SEARCH_SPACE:
        allowed = [str(value) for value in SEARCH_SPACE[knob]]
        if str(stock[knob]) not in allowed:
            outside.append("{} is {}, the space holds {}".format(knob, stock[knob], allowed))
    if outside:
        raise RuntimeError("this chip's own hierarchy is outside its search space:\n  "
                           + "\n  ".join(outside))
    return stock
...
def make_suite_problem(programs, allow_simulation=True, warmup=WARMUP_INSTRUCTIONS, simulation=SIMULATION_INSTRUCTIONS):
    ...
    def drawn(knobs):
        """What a design draws in watts over this suite. A rung the tables do not hold yet is
        measured now, once, where simulation is allowed, so every run decides the same stock:
        read from the tables alone, the stock depended on which run measured it first."""
        typed = typed_knobs(knobs)
        name = config_name(typed)
        metrics = {}
        for holder in holders:
            row = holder.sweep_table.get(name)
            if (row is None or row["metrics"] is None) and allow_simulation:
                holder.evaluate_many([typed])
                row = holder.sweep_table.get(name)
            if row is None or row["metrics"] is None:
                return None
            for key in row["metrics"]:
                metrics["{}:{}".format(holder.trace_name, key)] = row["metrics"][key]
        return watts(typed, metrics, names)
...
def aggregate_suite(per_workload_metrics, names, knobs):
    ...
    combined["watts_by_part"] = power_split(knobs, combined, names)
    combined["watts"] = sum(combined["watts_by_part"].values())
    return combined
