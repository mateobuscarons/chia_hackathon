...
2. **The BO baseline is tuned and the tuning is in the repo** (`loop/bo.py` WARM_UP_DESIGNS = 3; `loop/run.py` calls it at batch 1). Hyperparameters only: no memory, no warm start from cached tables.

   **The sweep.** All four knobs replayed offline against the cached tables at no simulation cost, 40 seeds, three candidate pools: `dc`'s 300-design uniform sample (the only pool that is a fair draw from the space) and the full measured set of `dc` and `dc2` (enriched with what earlier searches liked, so useful for ordering only). Each configuration scored by **area** — the mean gap share over D1..D16, not the endpoint, since a configuration that is poor for twelve designs and lucky at the end is not a better baseline — with the min..max over seeds printed beside it.

   **Only the initial design matters.** A warm-up of 3 or more is worth 6 area points on the fair pool and 8-10 on the enriched ones, and it cuts the seed spread at D16 from **29..100 to 59..100**: the untuned arm's catastrophic seeds are the ones where its cold first pick misleads the GP for the whole budget. Batch size, EI against logEI against UCB against greedy, and Matern 5/2 against 3/2 against an ARD RBF all land within 1-2 area points of each other — the three pools each crown a different batch/warm-up/kernel winner, every one of them by a point or less. Adopted: **warm-up 3, batch 1, expected improvement, the kernel unchanged** (Matern 5/2, bounds 0.05..5). Batch 1 is not an improvement in the numbers; it is chosen because every pick is then made against measured outcomes rather than the GP's guess at its own previous pick, which gives the baseline 16 adaptive decisions on a 16-design budget where the LLM arms get 8.

   **A correction to the earlier replay, which was wrong.** The first sweep used one fixed candidate pool for every seed, so a deterministic configuration produced the identical trajectory 40 times: that is where "batch 2 with no warm-up reaches 55% where random search reaches 87%" came from — one unlucky trajectory, not a distribution. The arm draws a **fresh candidate sample per seed** (`candidate_pool(problem, seed)`), and a replay that does the same reproduces the real arm: D16 86%, spread 29..100, against the actual `dc` run's 86%, 51..98. **The honest statement is that untuned BO matched random search on average and was wildly unreliable across seeds; tuning fixes the reliability, not the average.**

   **Nothing changed in the memory arm.** The sweep found nothing wrong with the ranking machinery (kernel, acquisition, batching), and those are the only parts `agent.select_by_gp` uses; the warm-up is a BO-only concept, because the memory arm's GP is never cold — its first designs are the LLM's own top picks. Separately, that GP was replayed in its actual job (ranking the 8 LLM candidates, 43 rounds recovered from the finished reports, scoring only candidates some run has since measured): it captures **55%** of each round's candidate spread against **45%** for taking the LLM's own order, top-1 19-26%, Spearman 0.18-0.28. So the GP filter is worth about ten points over the LLM's ranking, the kernel choice is within noise there too, and **the digest is contributing much more than the surrogate is**.
...
4. **Next, in this order.**
   ...
   **(4b) A fourth arm: BO started from the memory.** The tuned `bo` exactly as it stands, except that its first design is the one the memory hands over instead of a random warm-up design; same budget, seeds and cells. `bo.run_bo` does **not** take a start design today (an older note claimed it does) - the change is a start design threaded into `warm_up`, about twenty lines. Decided shape: the handover as D1 **plus** the tuned warm-up, so exactly one variable differs from `bo`. This is the arm that decides whether the LLM is load-bearing: if `memory` stays ahead, the LLM earns its place; if this arm ties or wins, the honest headline becomes "transfer works, the LLM is not what makes it work" and (4a) becomes the contribution. ArchGym's result ("all optimizers tie under tuned hyperparameters") reproduced in our own sweep, so this arm is the live question.

   **(4c) Tune the BO baseline - DONE**, see item 2 above. The sweep is scratch code (`bo_sweep.py`, `gp_select.py` in the session scratchpad); the outcome is in `loop/bo.py` and `loop/run.py`. One thing the sweep settled that was expected to go the other way: the winning configuration changed **nothing** in `agent.select_by_gp`, because the only knob that mattered was the initial design and the memory arm has no cold GP to warm up. So the two arms do not share a tuned surrogate; they share the same untouched one, and only the baseline's opening changed.
...
**BUDGET counts designs, not rounds** (`rounds = budget // PER_ROUND`), so the published cells at BUDGET=16 bought 16 designs over 8 rounds. The columns D1..D16 are designs. **The unit of cost is the design, the unit of adaptation is the round**: the two designs of a round are chosen together from the same information and listed in the arm's own rank order, so D1 is the design it ranked first and D2 is the better of that pair - not a step of learning. On `dc2` only 1 of 5 memory-arm seeds had its second design beat its first, which is the whole of that table's 81% -> 82%. The `bo` arm is the exception: it now buys one design per round, so every one of its columns is a fresh decision.
...
`upstream/` holds the patches. **The description of every contribution lives in `REPORT.md` section 4** (the `gs://` trace resolver, the config-space build node, the Vertex generation-config passthrough and its measured cost, the simulator-agnostic case memory, and the two spp_dev bugs). Do not restate them here.

## Rules of engagement

No dates or deadlines anywhere. **Code and docs state plain facts only: never name who will read or judge this work, and never write what a reviewer might say.** Anyone reading the repository should find the mechanism and the measurements, nothing about how the work expects to be received. Every launch needs the user's explicit OK with cost and time. The Mac runs only reference searches, overnight, under `caffeinate`. The VM is stopped when idle. Kill by process group, never a broad `pkill`. Code stays lean, one mechanism, no version archaeology; domain decisions are surfaced with options and left to the user. Deleted material is in git history, never in the tree.
...
python -m loop.forest 100 10 <trace> <trace> <trace>             # the ceiling for a suite: one long random-forest search (simulates)
...
```
Traces (`traces/`, gitignored): SPEC17 from `https://dpc3.compas.cs.stonybrook.edu/champsim-traces/speccpu/`; GAP from Zenodo record 20043527 with `loop.workloads fetch_gap` (cite *Characterizing the impact of last-level cache replacement policies on big-data workloads*, IISWC 2020); datacenter traces from the DPC4 bucket `https://pub-c31f67d79d1b4cd28ff320612b1a9f84.r2.dev/manifest.txt` (`gtrace_v2/<family>/<trace>.champsim.gz`, 359 traces in 12 families) as 100 MB prefixes with `loop.workloads fetch`. GCP: project `project-c23a6080-f5d0-4871-9cb`, VM `champsim-1` (c2d-standard-32, europe-west4-a, ~1.5 USD/h, 32-vCPU project cap), recipe in `cluster/README.md`. **The VM has no git checkout: code gets there with `gcloud compute scp loop/*.py`, never `git pull`.** Check `ls -l loop/bo.py` on it before a launch. The binding limit is the global `CPUS_ALL_REGIONS` quota of 32; europe-west4 itself allows 200 CPUS / 100 C2D_CPUS and the zone has c2d-standard-56 and -112, so a raise of that one quota would cut wall time proportionally at the same total cost (billing is per vCPU-hour) - refused while the billing account is on trial credits. Launch with `CHAMPSIM_BUILD_SHARE=8`: the default gives every build `make -j$(nproc)` while Ray allows 8 concurrent builds, so build waves oversubscribe the cores the simulations are using. Budget: **~80-90 EUR of credits left** — the GCP console is the source of truth, this line goes stale and must not be extrapolated from. Cost of a run, measured from every VM run so far: **94-154 designs simulated per hour, ~110 typical**, on the 32-vCPU VM at ~1.5 USD/h. Arms are not the cost, designs are: a 6-arm cell at budget 8 and 2 seeds is 81 designs and 42 minutes; a 3-arm cell at budget 16 and 5 seeds is 321 designs and 3.4 hours. Estimate a launch as (cells x arms x seeds x budget) / 110 hours.
