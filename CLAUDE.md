...
**A risk this exposes, needing a decision.** Turning on *any* L2 prefetcher buys +90% to +186% on two of the three held-out workloads, so every arm including `random` will capture most of the headroom within a design or two, compressing the differences between arms. The chip C baseline has no prefetcher at all (`BASELINE_KNOBS`), which is realistic as "the untouched chip" but makes the held-out search trivially easy at the start. Options: keep the identical baseline across cells for methodological cleanliness and report only designs-to-90/95/99% of the reference (already the plan), or give the held-out cell a prefetcher-enabled baseline so the search is not decided in one move. **Provisional held-out reference: 0.6110** (best of only 40 designs; a real reference needs broader coverage).
