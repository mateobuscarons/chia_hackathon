...
1. **A long run is on the VM, or has finished and powered the VM off: cell `dc`, tag `d3`, 16 designs, 5 seeds, six arms, `gemini-2.5-flash`.** Launched with `SMOKE_FIRST=1 POWER_OFF=1 SEEDS=5 BUDGET=16 bash cluster/launch_cell.sh dc d3`: the smoke runs first (`results/smoke_d3.log` on the VM), then the cell (`results/dc_d3.log`, report `results/run_dc_d3.json`), then the VM shuts itself down. Expected about 4 hours and 6-7 USD. **To collect it:** `gcloud compute instances start champsim-1 --zone europe-west4-a --project project-c23a6080-f5d0-4871-9cb`, then fetch `results/run_dc_d3.json`, `results/dc_d3.log`, `results/smoke_d3.log` and every `results/table_*.json` into a scratch dir, `python -m loop.workloads merge <dir>`, move the report and log into `results/`, `python -m loop.summarize results/run_dc_d3.json`, then **stop the VM again**. If the log ends in `SMOKE_FAILED`, the cell never ran. The VM's LLM cost is in `~/hackathon/loop/llm_usage.json` there.
