...
## Mechanism v5 (Sep 7, 2026, night): chip-independent conditions

**Why.** Conditions were thresholds on the target chip's own baseline metrics (MPKI, hit ratios). The same program has different MPKI on chips with different caches, so a rule learned on A ("L2C MPKI < 18") stayed silent on C (21.9) although its effect existed there (Tier C seed 0/1 autopsies: the LLC-prefetcher rule and Pro's +23% LLC-capacity rule were both mute on C). A reference-machine fingerprint was built and discarded the same evening (user: an imaginary machine, arbitrary probe sizes).

**What.** `loop/trace_profile.py`: the workload is profiled ONCE from its trace, no simulator: memory accesses per kinstr, write fraction, working set (footprint_kb), stride-regular fraction (byte-address strides per PC), reuse-local fraction, and a **miss-ratio curve** for any LRU capacity from footprint theory (Xiang et al., HOTL, ASPLOS 2013: exact fp(w) from the reuse-time histogram; miss ratio = slope). ~2 s per trace; cached in `results/profile_<trace>.json`. `champsim_problem.chip_descriptors` reads the profile against the target chip's public geometry (baseline sets x ways x 64 B): `l2_footprint_ratio`, `llc_footprint_ratio`, `pred_l1d/l2/llc_miss_ratio`. Conditions may use only these 10 descriptors (`DESCRIPTOR_METRICS`); `run_loop` checks speaking/silent/re-scope/widen on `problem["descriptors"]`; the distiller sees them in their own prompt section. Tier A (dense sweeps) keeps the old metrics.

**Validation (B baseline, no simulation used):** predicted vs ChampSim-measured miss ratio, L2 512KB: mcf 0.055/0.058, lbm 0.103/0.106, omnetpp 0.021/0.023, xalancbmk 0.004/0.008; LLC 2.5MB within 0.002 on all four. L1D prediction is ~2x low (48KB 12-way vs ideal LRU): ordinal use only. Note B and C share baseline cache geometry, so the same program gets identical descriptors on both; they differ in core/memory, which the claim tests measure.

**Also in v5 (same night, see commits):** silent rules get one claim test per chip and a won test widens the condition (mirror of re-scoping); stall scan also steps ordinal knobs; one-claim-one-rule enforced by the verifier; analyst prompts rewritten per Google's guide (system instruction, sections, one fictional example, temperature 0.3 for distill / 0.7 for hypotheses, thinking budget 8192 / 2048); `ANALYST_MODEL` env (Pro = `gemini-2.5-pro`, the only Pro available). Playbooks from before v5 are incompatible (old condition metrics): start fresh.
...
**Seed 0 result (`results/experiment_tierC_v2_C.json`, `python -m loop.summarize`):** designs to 90% of gain: **rules 7, full 12**, bo 18, bo_pooled >24, random >24; final best rules 0.7454 (= reference best), full 0.7423, bo 0.7316, bo_pooled 0.7292. Yesterday's seed 0: rules >24, full >24, bo_pooled 14. Calibration on shared dispute bets is weak for everyone (surrogate 0.50, rules 0.30 over 10, hypotheses 0.47) and should be looked at once seeds 1-2 are in. **(e) launched 20:16 in parallel (first attempt at 20:05 discarded after a prompt rewrite):** `ANALYST_MODEL=gemini-2.5-pro` (the only Pro the project can call; no Gemini 3 enabled) with rewritten prompts per Google's prompting guide (commit `2ece7aa`: role as system instruction; `##` sections, context first and task last; one fictional example per output type so shape is shown without content to copy; positive phrasing instead of a list of don'ts; distill/re-scope at temperature 0.3 with an 8192 thinking budget, hypotheses at 0.7/2048; measured one-knob effects listed largest first, one knob per rule). Fresh learn + seed 0 of all arms -> `results/experiment_tierC_v2pro_C.json`, log `tierC_v2pro.log`. Model and prompt changed together (user decision), so the seed-0 comparison flash-vs-pro is not a clean ablation of the model alone. Seeds 1-2 of all 5 arms launched 19:47 with the same playbook (`results/tierC_v2_s12.log`, output `experiment_tierC_v2_s12_C.json`, ~2 h, ~$3); table = `python -m loop.summarize results/experiment_tierC_v2_C.json results/experiment_tierC_v2_s12_C.json`.
