...
| Sep 8 (done) | Screening finished (25 traces), suites settled, **G-headroom passed** (0.83 USD, VM stopped). | see decisions 9-11 |
| Sep 9 | Free work: multi-core path for chip D; `summarize` fixes (per-cell reference, 90/95/99%, unfinished-arm crash); frozen `bo_pooled` pool. Then two paid collections, needing the user's OK: **(a)** `fotonik3d` + `cam4` columns for designs already in the A/B/C tables (~6 USD - incremental, the existing workloads are not re-simulated); **(b)** a proper held-out reference, ~300 designs x 3 GAP workloads (bounded by `bfs.urand` at ~0.7 min/design, so ~4-5 h, ~7 USD). | each suite has a reference over enough designs to make 90/95/99% meaningful |
