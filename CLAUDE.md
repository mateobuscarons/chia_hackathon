...
10. **Window length is now a precondition, not hygiene.** The same gate explains why: a workload is measurable only once its window is long enough to get past compulsory misses. gcc fails at 10M instructions; GAP graph traces have footprints far larger than gcc's 43 MB, so the headline cell would measure compulsory misses unless the window rises. The window must be chosen from the GAP profiles (gate G-window), and it multiplies the whole budget.
...
| Sep 9 | Screen candidate second capacity workload with `loop.offline admit` (each candidate: ~200 MB download + 2 s profile, zero cost). Download 1 GAP kernel and profile it at 10M / 50M / 200M windows before committing the GAP cell's budget. | **G-window:** the GAP kernels must have >= 1 movable MPKI at the chosen window. A graph trace's footprint is far larger than gcc's 43 MB, so at 10M instructions the GAP cell would measure compulsory misses and nothing else. This gate sets the simulation length, and therefore the budget. |
