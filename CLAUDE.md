...
3. **Open.** The duplicate-proposal fix: the LLM re-proposes already-measured designs, 4 to 9 of 8 per round; the retry recovers most, and a compact listing of measured designs in the task text would remove it. Then `gap2` (graph set 2) with the same three arms, Pro confirmation of the top arm, and the paper.
