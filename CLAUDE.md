...
1. **Cell `dc` d3 (16 designs, 5 seeds, six arms, flash) ran on the VM and powered it off; `bo`, `llm_direct` and `pooled` completed, all 15 memory-arm runs died** (report `results/run_dc_d3.json`, log `results/dc_d3.log`, tables merged, VM STOPPED). Cause, fixed afterwards: the 8-candidate answers exceeded CHIA's 16k output cap (thinking counts against it) and 15 parallel runs hit Vertex 429s, and CHIA's layer raises both at once; the CHIA call path now has a 65k cap and waits with backoff on any error (`chia_nodes.AnalystNode`, `agent.ask`). **Owed: rerun the three memory arms** (`SEEDS=5 BUDGET=16 SMOKE_FIRST=1 POWER_OFF=1 bash cluster/launch_cell.sh dc d3b memory,memory_pooled,memory_pooled_prior`, ~2 h, ~4 USD); `python -m loop.summarize results/run_dc_d3.json results/run_dc_d3b.json` merges the two reports. Collection steps for a finished run: start the VM, fetch report + logs + `results/table_*.json`, `python -m loop.workloads merge <dir>`, summarize, stop the VM.
1b. **d3 so far (share of the stock-to-best-known gap, mean of 5 seeds):** bo 43% at D1 -> 76% at D7 -> 86% at D16 (seeds 51..98); llm_direct 54% -> 82% at D4 -> 89% at D16 (85..96); pooled 94% in one design. The pooled default beats both search arms' 16-design means. The memory arms, before dying, all opened at 94% (they copy the pooled design) and two seeds were at 95% by design 4. Best design found: bo seed 0, 0.4906 (98%); best known stays 0.4927.
2. **The arms of d3** (`loop/run.py`, ARMS): `bo`; `llm_direct`; `memory` (LLM + digest with the closest workload's best design in the copyable slot + GP selecting 2 of its 8 proposals); `memory_pooled` (the same with the pooled design in the slot); `memory_pooled_prior` (the same with the selecting GP starting from the memory prior); `pooled` (one design, the bar). These were the user's choice: two ablations on top of the d2 winner, one changing what the memory hands over, one adding the statistical prior. `bo_prior`, `pooled_gp`, `llm_bo`, `replay` exist no more as arms (replay stays in the leave-one-out).
3. **Earlier runs on this cell, 8 designs, 2 seeds** (reports `run_dc_d1.json`, `run_dc_d2.json`, every prompt inside; `REVIEW.md` explains both for an outside reader). Scored against the best design known today (0.4927): d2 gave llm_bo_memory 95%, memory 90%, bo 88%, llm_direct 85%, replay 78%, llm_bo 74% of the stock-to-best gap after 8 designs. The pooled design alone reaches 94% in one design; one of its candidates is the 0.4927 best known, found by no search.
4. **What the runs taught, in order.** (a) The verification probe (d1) verified true things and spent the budget doing so; removed. (b) The raw case dump was unreadable; the digest (conclusions first) lifted the memory arm's first design from 40% to 65% of the gap. (c) GP selection among LLM proposals pays only when the proposals are good: with the digest it found the new best, without it the LLM proposed single-knob candidates and the GP could not rank them. (d) Pooled retrieval (best mean gap share across the memory, measured on all but one workload) beats nearest-case copying on 5 of 6 held-out rows and never falls below 82%; nearest falls to 27%. (e) A prior fit on raw log speed-ups ranks the datacenter designs at Spearman 0.29 because GAP's gains dominate; fit on gap shares, 0.73. (f) The hybrid arms re-propose measured designs, 4 to 9 of 8 per round; the retry recovers most; a compact listing of measured designs in the task text would remove it.
5. **Check the VM first** (`gcloud compute instances list`): it must be stopped whenever nothing runs on it. Free checks that need no VM: `python -m loop.memory leave_one_out <6 memory traces> -- <3 test traces>` (sign survival about two thirds, flat in distance; replay vs pooled columns) and `python -m loop.workloads headroom`.
...
- `memory_pooled`: the same, with the **pooled default** in the copyable slot: the remembered design with the best mean gap share across the memory workloads, counting only designs measured on all but one of them (`memory.pooled_design`, frozen in the memory file at build time). On the datacenter suite it reaches 94% of the gap in one design against 78% for nearest-case replay.
- `memory_pooled_prior`: the same, and the selecting GP starts from the **memory prior** and selects from round 1 (`bo.fit_prior`, `memory.prior_rows`): one GP fit once per run on every design the memory workloads measured (about 1,900 distinct designs), target = the design's share of each workload's stock-to-best gap in log-IPC units, averaged over the workloads that measured it and scaled by the median log gap; the run's GP fits only the residual. Shares, not raw speed-ups: on the 103 known datacenter designs the share prior ranks at Spearman 0.73, the speed-up prior at 0.29. The full stack.
- `pooled`: one design, the pooled default. The bar every search arm must clear.
One code path for the LLM arms: `agent.run_agent(memory_slot, use_gp, use_prior)`; with no memory the prompt is the plain agent's (checked).
