...
what either baseline reaches in 16 designs after about 12; the retired nearest-case digest variant
...
2. **The arms** (`loop/run.py`, ARMS): `bo`; `llm_direct`; `memory` (the digest, which hands over the pooled design, plus a GP selecting 2 of the LLM's 8 proposals). Everything else is retired: the nearest-case digest variant, the one-design `pooled` and `replay` arms, `bo_prior`, `pooled_gp`, `llm_bo` and `memory_pooled_prior`. `replay_rank` and `pooled_design` stay in the offline leave-one-out; the prior's negative result is recorded below.
