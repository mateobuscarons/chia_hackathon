...
**Both cells have run on the agent-built memory** (`results/run_dc_f1.json`, `results/run_dc2_f1.json`; reproduce with `python -m loop.summarize <report>`). `dc` is scored against an **independent** ceiling; `dc2`'s is still a by-product of earlier searches and will drop by roughly five points when its reference lands.
...
dc2  stock 0.4389, best known 0.5485 (NOT yet independent - reference running)
...
1. **When `g1` lands**: re-score both cells and compare `pooled_bo` against `memory`. If they tie, the memory is the contribution and the agent reading it is not — say so plainly and make the memory-building procedure the headline (`REPORT.md` §1 is already the measured case for it).
