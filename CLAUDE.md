...
**Retired: the GP prior on the memory tables.** One GP fit per run on every design the memory workloads measured (about 1,900 rows, target = each design's share of that workload's stock-to-best gap in log units), with the run's GP fitting only the residual. It ranked the known datacenter designs well on its own (Spearman 0.73 on shares, against 0.29 when fit on raw speed-ups) but added nothing as an arm: with the prior the memory arm matched its own 98% after 16 designs and was slightly slower in between. Removed from the code; the raw runs stay in `results/run_dc_d3b.json`. Other ways to bring a learned model into the loop are open, this particular one is answered.
