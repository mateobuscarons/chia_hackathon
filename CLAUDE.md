...
2. **The arms** (`loop/run.py`, ARMS): `bo`; `llm_direct`; `memory` (LLM + digest with the closest workload's best design in the copyable slot + GP selecting 2 of its 8 proposals); `memory_pooled` (the same with the pooled design in the slot); `pooled` (one design, the bar). `bo_prior`, `pooled_gp`, `llm_bo`, `memory_pooled_prior` and `replay` are retired as arms (replay stays in the leave-one-out; the prior's negative result is recorded below).
